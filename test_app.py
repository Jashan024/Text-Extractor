...
class TestRoutes:
    ...
    def test_extract_requires_json(self, client):
        resp = client.post("/extract", data="not json")
        assert resp.status_code == 400

    def test_extract_requires_text_key(self, client):
        resp = client.post("/extract",
                           data=json.dumps({"wrong": "key"}),
                           content_type="application/json")
        assert resp.status_code == 400

    def test_extract_rejects_empty_text(self, client):
        resp = client.post("/extract",
                           data=json.dumps({"text": "   "}),
                           content_type="application/json")
        assert resp.status_code == 400

    def test_extract_rejects_invalid_source(self, client):
        resp = client.post("/extract",
...
class TestIndeedParser:
    def test_single_profile(self, client):
        text = """John Smith
John Smith
Philadelphia, PA
...
Meta, 2019 - 2023"""
    ...
    def test_no_profiles_returns_empty(self, client):
    ...
    def test_profile_without_duplicate_name(self, client):
        text = """Jane Doe
Miami, FL
Relevant Work Experience
Project Manager
